...
function probe(shell: string, mode: "-il" | "-l"): Promise<Probe> {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      timedOut = true
      child.kill()
      // A shell that ignores SIGTERM (or a grandchild inheriting the stdout
      // pipe) can keep 'close' from ever firing; settle the probe here and
      // escalate so startup never waits on this promise indefinitely.
    ...
    child.on("close", (status) => {
      ...
      const env = parseShellEnv(Buffer.concat(chunks))
