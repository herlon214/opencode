...
const merge = async (app: Promise<LocaleModule>, desktop: Promise<LocaleModule>): Promise<Dictionary> => {
  const [appDict, desktopDict] = await Promise.all([app, desktop])
  return { ...base, ...i18n.flatten(appDict.dict), ...i18n.flatten(desktopDict.dict) }
}
