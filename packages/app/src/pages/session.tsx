...
export default function Page() {
  ...
  const desktopSidePanelOpen = createMemo(
    () => desktopSessionResizeOpen() || desktopFileTreeOpen() || !!layout.sideChat.sessionID(),
  )
  const sessionPanelWidth = createMemo(() => {
    if (!desktopSidePanelOpen()) return "100%"
    if (desktopSessionResizeOpen() || layout.sideChat.sessionID()) return `${layout.session.width()}px`
    return `calc(100% - ${layout.fileTree.width()}px)`
  })
  const centered = createMemo(
    () => isDesktop() && !desktopReviewOpen() && !layout.sideChat.sessionID(),
  )
    ...
    <SessionRouteFrame>
      ...
      >
        ...
        >
          ...
          <Show when={desktopSessionResizeOpen() || !!layout.sideChat.sessionID()}>
